 ...
 *
 *	This scheduler sends all packets exply on all available subflows.
 *
...
static struct sock *exp_get_subflow(struct sock *meta_sk,
					  struct sk_buff *skb,
					  bool zero_wnd_test)
{
	struct tcp_sock *meta_tp = tcp_sk(meta_sk);
...
static struct sk_buff *exp_next_skb_from_queue(struct sk_buff_head *queue,
						     ...
						     struct sock *meta_sk)
{
	 ...
	 *
	 * For the exp scheduler, there are two cases:
	 ...
	 *    with regard to the exp semantic, we have to
...
static struct sk_buff *exp_next_segment(struct sock *meta_sk,
					      int *reinject,
					      struct sock **subsk,
					      unsigned int *limit)
{
	...
	*limit = 0;
	...
	/* Then try indistinctly exp and normal skbs */
	...
	*reinject = 0;
	...
	do {
		...
		skb = exp_next_skb_from_queue(&meta_sk->sk_write_queue,
						    sk_data->skb, meta_sk);
...
static void exp_release(struct sock *sk)
{
...
static struct mptcp_sched_ops mptcp_sched_exp = {
	.get_subflow = exp_get_subflow,
	.next_segment = exp_next_segment,
	.release = exp_release,
	.name = "experimental",
	.owner = THIS_MODULE,
};
...
static int __init exp_register(void)
{
	...
	if (mptcp_register_scheduler(&mptcp_sched_exp))
		return -1;

	return 0;
}

static void exp_unregister(void)
{
	mptcp_unregister_scheduler(&mptcp_sched_exp);
}
